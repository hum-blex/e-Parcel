...
namespace e_Parcel.Controllers
{
	...
	public class PaymentDetailController : ControllerBase
	{
		...
		{
			var _data = await _unitOfWork.PaymentDetail.GetAllAsync(includeProperties: "Order");
		...
		{
			var _data = await _unitOfWork.PaymentDetail.GetAsync(filter: c => c.Id == id, includeProperties: "Order");
		...
		public async Task<IActionResult> Create([FromBody] PaymentDetailAddDto obj)
		{
			...
			var order = await _unitOfWork.OrderDetail.GetAsync(u => u.Id == obj.OrderId);
			if (order == null) return NotFound();
			var PaymentDetailDomain = _mapper.Map<PaymentDetail>(obj);
			PaymentDetailDomain.CreatedOn = DateTime.Now;
			PaymentDetailDomain.Status = obj.Status ?? "Pending";
			PaymentDetailDomain.OrderId = order.Id;
